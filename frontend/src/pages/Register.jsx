    ...
    <div className="mt-2 space-y-1">
      {checks.map((c) => (
        <div key={c.label} className="flex items-center gap-2 text-xs">
          <span className={c.ok ? 'text-green-400' : 'text-gray-600'}>
            {c.ok ? '✓' : '○'}
          </span>
    ...
    <div className="min-h-screen bg-[#0a0a0f] flex">
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-indigo-950 via-[#0d0d1a] to-[#0a0a0f] flex-col justify-between p-12">
        ...
        <div>
          <div className="space-y-4 mb-12">
            {[
              { icon: '🤖', title: 'AI Probability Analysis', desc: 'DeepSeek AI membaca momentum market Polymarket secara berkala' },
              { icon: '🛡️', title: 'Risk Management', desc: 'Strategy SL/TP, loss limit, dan tracking drawdown' },
              { icon: '📊', title: 'Real-time Dashboard', desc: 'Monitor strategy stats, signals, dan riwayat market' },
            ].map((f) => (
          ...
          <h2 className="text-3xl font-bold text-white mb-3">Trading lebih cerdas<br />dengan sinyal prediction market.</h2>
          <p className="text-gray-500">Gabung sekarang dan mulai eksplorasi data Polymarket dengan AI.</p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          ...
          <p className="text-gray-500 text-sm mb-8">Gratis. Tidak perlu kartu kredit.</p>

          {error && (
            <div className="mb-4 flex items-start gap-3 bg-red-500/10 border border-red-500/20 text-red-400 text-sm px-4 py-3 rounded-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                value={form.email}
                onChange={set('email')}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 transition-all"
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-gray-400 uppercase tracking-wider">Konfirmasi Password</label>
